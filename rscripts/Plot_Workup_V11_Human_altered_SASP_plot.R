...
condition <- read.csv("Human_Search/230602_RCCN2_bothAB_Kidney_Cortex_ConditionSetup_v02.csv",
                      stringsAsFactors = F)
...
proIDs <- paste(comma(nrow(protein)), 
                "NHP Homologous Protein Groups Identified with ≥ 2 Unique Peptides",
                sep = " ")
...
pca_heat_corr <- function(df, pca_name, heat_name, corr_name, mycol){
  ...
  tiff(file = heat_name, res = 300, width = 7, height = 7, units = "in")
  heatmap.2(t(scale(t(log10(pcMat)))), col = myRamp, trace = 'none', labRow = FALSE,
            ColSideColors = myheatcol, cexRow = 0.8, cexCol = 0.8)
  dev.off()
...
Core_SASP <- function(pro_heat_name, pro_SASP_name, 
  ...
  my_SASP_comp <- comparisons[comparisons$Genes %in% SASP$Genes == TRUE,]
  ...
  df <- df[df$Comparison..group1.group2.%in% interest == TRUE,]
  df$Color <- ifelse(df$AVG.Log2.Ratio >= myFC & df$Qvalue < myQval, "Red", "Grey")
  df$Color <- ifelse(df$AVG.Log2.Ratio <= -myFC & df$Qvalue < myQval, "Blue", df$Color)
  df <- df %>% filter(!(AVG.Log2.Ratio >= -0.58 & AVG.Log2.Ratio <= 0.58))
  ...
  ## Prepare for heatmap
  temp <- dplyr::select(df, contains(c("AVG")))
  temp <- temp %>% mutate_all(~replace(., is.na(.), 0))
  temp <- data.matrix(temp)
  rownames(temp) <- df$Genes
  colnames(temp) <- gsub("AVG.Log2.Ratio_","",colnames(temp))
  
  tiff(file = "output/heatmaps/RCCN2_SASP_12122023_v01.tiff", res = 300, height = 8, width = 8, units = "in")
  heatmap.2(temp, col = myRamp, trace = 'none', 
            labRow = row.names(temp), labCol = colnames(temp), margins= c(13, 5))
  dev.off()
...
Matrisome <- function(my_Matrisome, my_Hist){
  ...
  my_ECM <- right_join(comparisons, ECM, by = c("Genes" = "Gene.Symbol"))
